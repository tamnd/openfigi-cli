...
describes what users get. Pick a format with `-o`, or let openfigi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
openfigi <command> -o table   # aligned columns for reading
openfigi <command> -o jsonl   # one JSON object per line, for piping
openfigi <command> -o json    # a single JSON array
openfigi <command> -o csv     # spreadsheet friendly
openfigi <command> -o tsv     # tab-separated
openfigi <command> -o url     # just the URL column
openfigi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
openfigi <command> --fields id,title,url
```
...
```bash
openfigi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
openfigi <command>            # a table, because this is a terminal
openfigi <command> | wc -l    # JSONL, because this is a pipe
```
