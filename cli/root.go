// Package cli assembles the openfigi command tree from the openfigi
...
// NewApp assembles the kit application from the openfigi domain. The
...
//
// To add a command, declare it in openfigi/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := openfigi.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(openfigi.Domain{}).Register(app)
